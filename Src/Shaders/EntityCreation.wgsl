...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    atomicAdd(&b_Entity[0], 1); //ioCount
    let prev_free_count: i32 = atomicAdd(&b_Entity[1], -1);//ioFreeCount
    atomicMax(&b_Entity[1], 0); // ensure never negative
    let new_id: i32 = atomicAdd(&b_Entity[2], 1); //ioNextId
