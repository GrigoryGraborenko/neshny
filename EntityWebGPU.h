...
void SerializeStructInfo(StructInfo& info, QString get_base_str, QString entity_name) {
	...
	read_only_lines += QString("fn Get%1(index: i32) -> %1 {").arg(entity_name);
	read_only_lines += get_base_str;
	read_only_lines += QString("\tvar result: %1;").arg(entity_name);
	int pos_index = 0;
	QStringList functions;
	for (auto member : info.p_Members) {
		QString get_syntax = MemberSpec::GetGPUGetSyntax(member.p_Type, pos_index, entity_name);
		read_only_lines += QString("\tresult.%1 = %2;").arg(member.p_Name).arg(get_syntax);
		functions += QString("fn Get%3%2(index: i32) -> %1 {\n").arg(MemberSpec::GetGPUType(member.p_Type)).arg(member.p_Name).arg(entity_name) + get_base_str + QString("\n\treturn %1;\n}").arg(get_syntax);
		if (member.p_Type == MemberSpec::Type::T_INT) {
			functions += QString("// defined macro Access%2%1(index)\n").arg(member.p_Name).arg(entity_name);
	...
	lines += QString("fn Set%1(item: %1, index: i32) {").arg(entity_name); // TODO: make this a ref?
	lines += get_base_str;
	pos_index = 0;
	...
	for (auto member : info.p_Members) {
		...
		if (member.p_Type == MemberSpec::T_INT) {
			...
			mod_str = QString("\t%3_SET(base, %1, bitcast<i32>(item.%2));").arg(pos_index).arg(name).arg(entity_name);
			value_mod_str = QString("\t%2_SET(base, %1, bitcast<i32>(value));").arg(pos_index).arg(entity_name);
		} else if (member.p_Type == MemberSpec::T_VEC2) {
			mod_str = QString("\t%4_SET(base, %1, bitcast<i32>(item.%3.x)); %4_SET(base, %2, bitcast<i32>(item.%3.y));").arg(pos_index).arg(pos_index + 1).arg(name).arg(entity_name);
			value_mod_str = QString("\t%3_SET(base, %1, bitcast<i32>(value.x)); %3_SET(base, %2, bitcast<i32>(value.y));").arg(pos_index).arg(pos_index + 1).arg(entity_name);
		} else if (member.p_Type == MemberSpec::T_VEC3) {
			mod_str = QString("\t%5_SET(base, %1, bitcast<i32>(item.%4.x)); %5_SET(base, %2, bitcast<i32>(item.%4.y)); %5_SET(base, %3, bitcast<i32>(item.%4.z));").arg(pos_index).arg(pos_index + 1).arg(pos_index + 2).arg(name).arg(entity_name);
			value_mod_str = QString("\t%4_SET(base, %1, bitcast<i32>(value.x)); %4_SET(base, %2, bitcast<i32>(value.y)); %4_SET(base, %3, bitcast<i32>(value.z));").arg(pos_index).arg(pos_index + 1).arg(pos_index + 2).arg(entity_name);
		} else if (member.p_Type == MemberSpec::T_VEC4) {
			mod_str = QString("\t%6_SET(base, %1, bitcast<i32>(item.%5.x)); %6_SET(base, %2, bitcast<i32>(item.%5.y)); %6_SET(base, %3, bitcast<i32>(item.%5.z)); %6_SET(base, %4, bitcast<i32>(item.%5.w));").arg(pos_index).arg(pos_index + 1).arg(pos_index + 2).arg(pos_index + 3).arg(name).arg(entity_name);
			value_mod_str = QString("\t%5_SET(base, %1, bitcast<i32>(value.x)); %5_SET(base, %2, bitcast<i32>(value.y)); %5_SET(base, %3, bitcast<i32>(value.z)); %5_SET(base, %4, bitcast<i32>(value.w));").arg(pos_index).arg(pos_index + 1).arg(pos_index + 2).arg(pos_index + 3).arg(entity_name);
		...
		lines += mod_str;
		functions += QString("fn Set%3%1(index: i32, value: %2) {\n").arg(member.p_Name).arg(MemberSpec::GetGPUType(member.p_Type)).arg(entity_name) + get_base_str + QString("\n%1\n}").arg(value_mod_str);
