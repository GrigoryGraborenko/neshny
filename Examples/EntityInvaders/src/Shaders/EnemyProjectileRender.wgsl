...
fn vertex_main(input : VertexIn) -> VertexOut {
	...
	let enemy = GetEnemyProjectile(i32(input.Instance));
	if (enemy.Id < 0) {
		output.Position = vec4f(0.0);
		return output;
	}
	
	output.Position = Uniform.ViewPerspective * vec4f(input.Pos * 0.05 + enemy.Pos, 0.0, 1.0);
...
fn frag_main(input: VertexOut) -> @location(0) vec4f {
	...
	let col = vec3f(1.0, 0.3, 0.0) * mult;
